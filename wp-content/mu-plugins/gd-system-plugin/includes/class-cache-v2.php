...
final class Cache_V2 {
	...
	public function __construct( API_Interface $api ) {
		...
		add_action( 'customize_save', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		// Theme/Plugin change
		add_action( 'switch_theme', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'activated_plugin', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'deactivated_plugin', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		// Core upgrade
		add_action( '_core_updated_successfully', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'upgrader_process_complete', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		// Navigation
		add_action( 'wp_update_nav_menu', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'wp_delete_nav_menu', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'wp_create_nav_menu', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		// Post publish/unpublish: full BAN to invalidate every cached URL where the post may appear.
		// Selective do_purge() can only enumerate the standard archive URLs, so content-driven
		// list pages (Latest Posts blocks, query loops, custom CPT listings on regular Pages, paginated
		// views) would otherwise stay stale until the user manually flushes cache.
		add_action( 'transition_post_status', [ $this, 'do_ban_on_publish' ], PHP_INT_MAX, 3 );
		...
		// Taxonomy
		add_action( 'created_category', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'created_post_tag', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'edited_category', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'edited_post_tag', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'delete_category', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
		add_action( 'delete_post_tag', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );

		// Launch Plugin
		add_action( 'gdl_splash_page_set', [ $this, 'do_ban_no_flush' ], PHP_INT_MAX, 0 );
	...
	public function do_ban() {
		...
		remove_action( 'shutdown', [ $GLOBALS['wpaas_cache_class'], 'ban_no_flush' ], PHP_INT_MAX );
	...
	/**
	 * Set to ban cache on shutdown WITHOUT flushing object cache.
	 *
	 * Called by WordPress hooks where cache has already been cleared by WordPress.
	 * This prevents double-flushing when WordPress core already cleared the caches.
	 */
	public function do_ban_no_flush() {

		if ( $this->has_ban() || $this->has_ban_no_flush() ) {
			return;
		}

		remove_action( 'shutdown', [ $GLOBALS['wpaas_cache_class'], 'purge' ], PHP_INT_MAX );
		add_action( 'shutdown', [ $GLOBALS['wpaas_cache_class'], 'ban_no_flush' ], PHP_INT_MAX );
	}

	/**
	 * Trigger a full cache BAN on actual publish-state transitions.
	 *
	 * Fires on draft/future/trash → publish (post going live), publish → publish
	 * (updates to a live post), and publish → draft/trash (unpublishing). Skips
	 * revisions and any transition that does not touch the publish state, so
	 * draft saves, autosaves and scheduling are unaffected.
	 *
	 * Selective do_purge() cannot enumerate URLs derived from content-driven
	 * layouts (Latest Posts blocks, query loops, custom CPT listings hosted on
	 * regular Pages, paginated views), so a BAN is the only reliable way to
	 * invalidate them on publish.
	 *
	 ...
	 * @param \WP_Post $post
	 */
	public function do_ban_on_publish( $new_status, $old_status, $post ) {
	...
	public function flush_object_cache() {
		/**
		 * Allow skipping object cache flush for testing purposes.
		 * Default: false (cache flush enabled)
		 * Set to true to disable: WPAAS_SKIP_OBJECT_CACHE_FLUSH=true
		 */
		if ( defined( 'WPAAS_SKIP_OBJECT_CACHE_FLUSH' ) && WPAAS_SKIP_OBJECT_CACHE_FLUSH ) {
			return;  // Skip flushing
		}

		// Full flush when explicitly requested (user manual cache clear)
		wp_cache_flush();
	}
	...
	/**
	 * Check if a BAN_NO_FLUSH request is already set to fire on shutdown.
	 *
	 * @return bool
	 */
	public function has_ban_no_flush() {
		return has_action( 'shutdown', [ $GLOBALS['wpaas_cache_class'], 'ban_no_flush' ] );
	}
	 ...
	 * @return bool
	 */
	public function ban(bool $flush_object_cache = true) {
	...
	/**
	 * Ban all cache WITHOUT flushing object cache (async).
	 *
	 * Called when WordPress hooks have already cleared the relevant object caches.
	 * This bans Varnish/CDN without redundantly flushing object cache.
	 *
	 * @return bool
	 */
	public function ban_no_flush() {
		// Ban Varnish/CDN but DON'T flush object cache (WordPress already cleared it)
