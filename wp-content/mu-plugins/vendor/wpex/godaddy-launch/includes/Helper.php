...
class Helper {
	...
	/**
	 * Return the hosting dashboard (MYH) host for the current brand.
	 *
	 * GoDaddy-direct sites (GD_RESELLER = 1 or undefined) use host.godaddy.com.
	 * Reseller brands (123 Reg, Host Europe, etc.) use host.secureserver.net.
	 *
	 * @return string
	 */
	public static function myh_host() {
		$is_reseller_v1 = defined( 'GD_RESELLER' ) && 1 !== (int) constant( 'GD_RESELLER' );

		$config_data    = defined( 'configData' ) ? json_decode( constant( 'configData' ), true ) : array();
		$is_reseller_v2 = array_key_exists( 'GD_RESELLER', $config_data ) && '1' !== $config_data['GD_RESELLER'];

		$host = ( $is_reseller_v1 || $is_reseller_v2 ) ? 'host.secureserver.net' : 'host.godaddy.com';

		return (string) apply_filters( 'godaddy_launch_myh_host', $host );
	}
	...
	public static function get_default_nux_api_request_body() {
		...
		return array(
			...
			'language'              => substr( get_user_locale(), 0, 5 ),
