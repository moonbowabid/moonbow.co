...
class PublishGuideServiceProvider extends ServiceProvider {
		...
		if ( \is_plugin_active( 'ameliabooking/ameliabooking.php' ) ) {
			$excluded_pages = array_merge(
				...
				array(
					...
					'wpamelia-customers',
					'wpamelia-finance',
					'wpamelia-notifications',
					'wpamelia-employees',
					'wpamelia-customize',
					'wpamelia-cf',
					'wpamelia-settings',
					'wpamelia-whats-new',
					'wpamelia-lite-vs-premium',
				)
			);
		}
	...
	public function get_change_domain_uri() {
		...
		$myh_host = Helper::myh_host();

		return GD_TEMP_DOMAIN
			? 'https://' . $myh_host . '/mwp/siteLookup/?domain=' . GD_TEMP_DOMAIN . '&path=changedomain'
			: 'https://' . $myh_host . '/mwp/siteLookup/?domain={{DOMAIN}}&path=changedomain';
	}
